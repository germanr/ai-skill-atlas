...
function approvedEditorialIds(reviews, previousReleaseId, previous, current, errors) {
  const approved = new Set();
  const seen = new Set();
  if (!Array.isArray(reviews)) { errors.push("Editorial receipts must be an array."); return approved; }
  for (const receipt of reviews) {
    if (!objectRecord(receipt) || !nonempty(receipt.previous_release_id) ||
      !nonempty(receipt.estimate_id) || receipt.field !== "notes" ||
      typeof receipt.before !== "string" || typeof receipt.after !== "string" ||
      receipt.before === receipt.after || !nonempty(receipt.reason)) {
      errors.push("Editorial receipt requires a previous release, estimate ID, exact changed notes, and a reason.");
      continue;
    }
    const key = `${receipt.previous_release_id}:${receipt.estimate_id}`;
    if (seen.has(key)) { errors.push(`${receipt.estimate_id}: duplicate editorial receipt for ${receipt.previous_release_id}.`); continue; }
    seen.add(key);
    // Older receipts remain in the audit ledger but cannot approve a later edit.
    if (receipt.previous_release_id !== previousReleaseId) continue;
    const old = previous.get(receipt.estimate_id);
    const next = current.get(receipt.estimate_id);
    if (!old || !next) {
      errors.push(`${receipt.estimate_id}: editorial receipt requires an existing estimate in both releases.`);
      continue;
    }
    if (old.notes !== receipt.before || next.notes !== receipt.after) {
      errors.push(`${receipt.estimate_id}: editorial receipt does not match the exact old and new notes.`);
      continue;
    }
    const withoutNotes = ({ notes, ...record }) => record;
    if (JSON.stringify(canonical(withoutNotes(old))) !== JSON.stringify(canonical(withoutNotes(next)))) {
      errors.push(`${receipt.estimate_id}: editorial receipt cannot approve changes outside notes.`);
      continue;
    }
    approved.add(receipt.estimate_id);
  }
  return approved;
}

// Compare raw release records, not resolver-enriched display records. Unchanged
// legacy rows keep their honest unknowns. Exact editorial receipts cover only
// approved notes revisions, not source verification or any other field change.
export function validateMetadataRelease({
  papers = [], estimates = [], previousEstimates = [], metadata = {},
  previousReleaseId = null, editorialReviews = [],
}) {
  const errors = [];
  ...
  const editorialIds = approvedEditorialIds(editorialReviews, previousReleaseId, previous, estimateMap, errors);
  const editorialReviewIds = [];
  ...
  for (const estimate of estimates) {
    ...
    if (editorialIds.has(id)) { editorialReviewIds.push(id); continue; }
  ...
  return { valid: errors.length === 0, errors, requiredReviewIds, editorialReviewIds };
}
