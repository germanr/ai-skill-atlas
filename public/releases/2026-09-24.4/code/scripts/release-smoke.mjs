...
const editorialLedger = readJSON(join(root, "code", "audit", "editorial-reviews.json"));
const appliedReceipts = editorialLedger.reviews.filter(receipt => receipt.previous_release_id === existing.id);
assert.equal(manifest.editorial_review.estimate_count, appliedReceipts.length);
assert.deepEqual([...manifest.editorial_review.estimate_ids].sort(), appliedReceipts.map(receipt => receipt.estimate_id).sort());
assert.ok(manifest.files.some(file => file.source_path === "code/audit/editorial-reviews.json"));
assert.deepEqual(readJSON(join(fixture, "public", "releases", entry.id, manifest.editorial_review.receipt_path)), editorialLedger);
for (const receipt of appliedReceipts) {
  assert.equal(manifest.source_review.newly_reviewed_estimate_ids.includes(receipt.estimate_id), false,
    "Editorial changes must not be labeled fresh source reviews.");
  const prior = readJSON(join(root, "public", "releases", existing.id, "data", "estimates.json"))
    .find(row => row.estimate_id === receipt.estimate_id);
  const next = readJSON(join(fixture, "public", "releases", entry.id, "data", "estimates.json"))
    .find(row => row.estimate_id === receipt.estimate_id);
  const { notes: beforeNotes, ...beforeFields } = prior;
  const { notes: afterNotes, ...afterFields } = next;
  assert.equal(beforeNotes, receipt.before);
  assert.equal(afterNotes, receipt.after);
  assert.deepEqual(afterFields, beforeFields, "An editorial receipt changed a non-notes field.");
}
...
// Original source/sample review statuses and dates survive prose-only edits.
const priorMetadata = readJSON(join(root, "public", "releases", existing.id, "data", "evidence_metadata.json"));
for (const receipt of appliedReceipts) {
  for (const part of ["sample", "provenance"]) {
    for (const field of ["review_status", "review_basis", "review_date", "extraction_date"]) {
      assert.equal(metadata.estimates?.[receipt.estimate_id]?.[part]?.[field],
        priorMetadata.estimates?.[receipt.estimate_id]?.[part]?.[field],
        `${receipt.estimate_id}: editorial changes must preserve ${part}.${field}`);
    }
  }
}
