...
const CODE_PATHS = [
  ...
  "code/build_website_data.py", "code/build_creativity_data.py", "code/patch_subgroups.py", "code/estimand_method.json",
  "scripts/release.mjs", "scripts/smoke.mjs", "scripts/browser-smoke.mjs",
  "scripts/view-state-smoke.mjs", "scripts/metadata-smoke.mjs", "scripts/release-smoke.mjs",
  "scripts/audit-smoke.mjs", "scripts/social-card.mjs", "scripts/social-card.html", "public/images/og-card.meta.json",
  "code/sources/contractor-reyes-2026-09.json", "code/audit/bassner_gain_precision.do",
  "code/audit/bassner_gain_precision.csv", "code/audit/bassner_arm_statistics.csv",
  "code/audit/editorial-reviews.json",
  "package.json", "package-lock.json", "vite.config.js", "index.html", "about/index.html",
  "README.md", ".gitattributes",
];
const digest = bytes => createHash("sha256").update(bytes).digest("hex");
const sourceDigest = bytes => digest(Buffer.from(bytes.toString("utf8").replace(/\r\n/g, "\n")));
...
export function verifyRelease(root, entry, { current = false } = {}) {
  ...
  assert(manifest.schema_version === SCHEMA_VERSION && manifest.id === entry.id && manifest.date === entry.date &&
    manifest.label === entry.label && manifest.kind === entry.kind,
  ...
  for (const file of manifest.files) {
    ...
    if (file.source_sha256) {
      assert(file.source_hash_normalization === "utf8_crlf_to_lf" && sourceDigest(bytes) === file.source_sha256,
        `${entry.id}: normalized source hash failed for ${file.path}.`);
    }
    if (current && file.source_path) {
      const currentBytes = readFileSync(safePath(root, file.source_path));
      const currentHash = file.source_sha256 ? sourceDigest(currentBytes) : digest(currentBytes);
      assert(currentHash === (file.source_sha256 || file.sha256), `${entry.id}: current ${file.source_path} differs from its release snapshot. Create a new release after review.`);
    }
  }
  ...
  if (manifest.editorial_review) {
    const review = manifest.editorial_review;
    const known = new Set([...rowsOf(data.estimates), ...rowsOf(data.creativity_estimates)]
      .map(row => row.estimate_id || `${row.paper_key}__outcome_${row.outcome_idx}`));
    assert(Array.isArray(review.estimate_ids) && review.estimate_count === review.estimate_ids.length &&
      new Set(review.estimate_ids).size === review.estimate_ids.length && review.estimate_ids.every(id => known.has(id)),
      `${entry.id}: invalid editorial review count or estimate IDs.`);
    assert(review.estimate_ids.every(id => !manifest.source_review?.newly_reviewed_estimate_ids?.includes(id)),
      `${entry.id}: editorial changes cannot be labeled new source reviews.`);
    assert((review.estimate_count === 0 || review.receipt_path) &&
      (!review.receipt_path || manifest.files.some(file => file.path === review.receipt_path && file.source_path === "code/audit/editorial-reviews.json")),
      `${entry.id}: editorial receipt ledger is missing from the snapshot.`);
  }
  if (current) {
...
export function createRelease({
  ...
  let editorialReviewIds = [];
  const editorialReceiptPath = "code/audit/editorial-reviews.json";
  let editorialReviews = [];
  if (existsSync(join(sourceRoot, editorialReceiptPath))) {
    const ledger = readJSON(join(sourceRoot, editorialReceiptPath));
    assert(ledger.schema_version === 1 && Array.isArray(ledger.reviews), "Invalid editorial receipt ledger.");
    editorialReviews = ledger.reviews;
  }
  if (kind !== "baseline") {
  ...
  const manifest = {
    ...
    editorial_review: {
      estimate_count: editorialReviewIds.length,
      estimate_ids: editorialReviewIds,
      receipt_path: editorialReviews.length ? `code/${editorialReceiptPath}` : null,
      note: "Exact approved notes revisions only. All other estimate fields are unchanged. Editorial review does not verify sources or change source/sample review status.",
    },
    reproducibility: {
      ...
      restore: "For calculations and exports, use code/src/shared.mjs with data/*.json; snapshot-tools contains the exact CSV generator and its dependencies. This is not a complete standalone website backup. For the app, copy data/code files to their manifest source_path in a separate directory, copy this entire archive to public/releases/<id>, reconstruct src/release.json from this release's published index entry, and create public/releases/index.json with that entry/current_release_id/schema_version. The reconstructed pointer/index are intentionally outside their own hash manifest to avoid a circular digest. Historical archive lists, PDFs, images and other site assets are not bundled. Install with the archived package-lock.json using npm ci. Do not overwrite a live checkout to reproduce an older release.",
    },
    files: payloads.map(({ buffer, ...file }) => ({ ...file, bytes: buffer.length, sha256: digest(buffer),
      ...(file.source_path ? { source_hash_normalization: "utf8_crlf_to_lf", source_sha256: sourceDigest(buffer) } : {}),
    })),
  };
