"""Build website data from research inputs and versioned website corrections.

Outputs:
  1. papers_for_website.xlsx (in support_info/meta_analysis/) — optional derived export.
  2. src/papers.json — bundled into the React app.
  3. src/estimates.json — bundled into the React app.

Inputs: meta_analysis.xlsx, literature_effects.csv, the corrections below,
estimand_method.json, and a reviewed Contractor-Reyes source snapshot.
Ordinary builds never read the changing regression_results_main.dta.

Run with:
  python code/build_website_data.py --json-only
  python code/build_website_data.py --json-only --output-dir C:/path/to/preview
"""
...
# ── paths ──────────────────────────────────────────────────────────────────
RESEARCH = Path("C:/Dropbox/Research/ai-learning")
...
OWN_SOURCE = SITE / "code" / "sources" / "contractor-reyes-2026-09.json"
BASSNER_PRECISION = SITE / "code" / "audit" / "bassner_gain_precision.csv"
...
def reader_estimate_notes(notes: str | None) -> str | None:
    """Apply reviewed wording edits without changing source snapshots."""
    if not isinstance(notes, str):
        return notes
    replacements = (
        ("Stata 18 read the nine OLS rows without modifying the research dataset. ", ""),
        ("Retain the outcome-specific N=196; treatment/control counts are unverified and left null.",
         "The estimate uses the outcome-specific N=196; treatment/control counts are unverified and left null."),
        (" -- essentially no effect.", "."),
        (" Free AI use was no better than slide-only review.", ""),
        (" [Codex audit note, 2026-07]", ""),
        (" [Codex audit fix, 2026-07]", ""),
        ("KEY: Unrestricted GPT-4 during practice => 17% worse on unassisted exam.",
         "Students with unrestricted GPT-4 access during practice scored 17% lower on the unassisted exam."),
        ("Low-rated tutors + CoPilot = high-rated tutors without it.",
         "Estimated exit-ticket pass rates were similar for students of lower-rated tutors using CoPilot and higher-rated tutors without it."),
        ("; no MCQ effect (", "; the multiple-choice omnibus test was not statistically significant ("),
        (" and absent among high-spatial-ability students (76.58 vs 73.89, P=.22).",
         ". The difference among high-spatial-ability students was not statistically significant (76.58 vs 73.89, P=.22)."),
    )
    for source, replacement in replacements:
        notes = notes.replace(source, replacement)
    return notes


# ── manual mappings ────────────────────────────────────────────────────────
# For each paper, hand-curated columns that aren't cleanly derivable from
# the raw meta_analysis.xlsx. Edit these inputs/corrections, not the derived
# papers_for_website.xlsx export, to make a change survive regeneration.
...
PAPER_META = {
    "Franco et al. (2026)": dict(
        ...
        summary="Pre-registered lab experiment at Nottingham randomizing 604 students to browsing-only, ChatGPT, or ChatGPT-plus-guidance while studying Esperanto, with 572 students analyzed. No average effect on an unaided exam; AI crowds out practice questions, and heavy copy-pasters learn least.",
    ...
    "Fan et al. (2025)": dict(
        ...
        summary="University students wrote essays with ChatGPT vs checklist, expert, or no support. AI users produced better essays but showed no significant knowledge gain or transfer advantage; the authors interpret process data as evidence of \"metacognitive laziness\".",
    ...
    'Hou et al. (2026)': dict(
        ...
        summary='An online three-arm RCT with 95 undergraduates testing whether a five-step prompting framework changes what students learn from a course-grounded (RAG) generative-AI assistant. Guided AI use raised unassisted post-test scores (d~0.86), mainly through open-ended reasoning items, while free-form AI use produced a small estimated difference from slide-only review.',
...
# The eligibility threshold concerns the full trial, not a contrast's N.
# Kalam: linked Cureus article (June 11, 2025), DOI 10.7759/cureus.85767,
# abstract p.1 and Table 1 p.4: 33 randomized across three arms (10/12/11).
# Its 21-person ChatGPT-versus-control contrast was not the trial total.
# Excluded September 23, 2026 under the atlas's >=50-participant criterion.
# Previous releases remain unchanged, including the two excluded estimates.
EXCLUDED_PAPER_REASONS = {
    "Wu et al. (2025)": "Odd/even alternation, not random assignment or a clean quasi-experiment.",
    "Kalam et al. (2025)": "33 randomized participants, below the 50-participant minimum (Cureus Table 1).",
}
DROP_PAPERS = set(EXCLUDED_PAPER_REASONS)
...
PAPER_CORRECTIONS = {
    "franco_etal_2026": dict(
        ...
        quality_flags="Exam-score SD not verified; raw exam-point coefficients are retained in estimate notes, with no SD conversion.",
    ),
...
PAPER_SUMMARIES = {
    "franco_etal_2026": {
        ...
        "key_results": "No statistically significant average effect on exam scores in either AI arm (about 8 of 15 correct in all arms). Adjusted effects are +0.078 exam points (SE 0.192) for AI-assisted access and -0.234 points (SE 0.189) for AI-guided access, on a 0-15 scale (Table 2). No verified exam-score SD is available for standardization. AI arms attempted 4.2-5.8 fewer practice questions (12-17 percent less) at a higher per-question success rate. Suggestive heterogeneity: high-GPA women gain under guided access while low-GPA students face a higher bottom-tail risk. The heaviest copy-pasters score 16 percent below the control mean; high-GPA women copy-paste least.",
    },
    ...
    "fan_etal_2025": {
        ...
        "key_results": "AI group's essay improvement significantly exceeded all three other arms (d≈0.73 vs CN; F=4.55, p=0.005). But no significant group differences in knowledge gain (d≈-0.05) or knowledge transfer. Trace-data process mining showed that AI students repeatedly revised through ChatGPT rather than reading and evaluating. The authors describe this pattern as 'metacognitive laziness'.",
    },
    "hausman_etal_2025": {
    ...
    "kumar_etal_2023": {
        ...
        "key_results": "LLM explanations boosted test accuracy. Largest gains in Try First: ~50% correct in Answer only vs >67% in Stock LLM (z=-3.46, p<0.001) and Customized LLM (z=-4.20, p<0.001). In See Answer First, only Customized LLM beat Answer only marginally. Participants’ strategy reports suggest that adopting strategies shown by the LLM contributed to the gains. LLM-condition participants reported lower perceived difficulty and >85% felt they learned something.",
    },
    "learnlm_team_2025": {
    ...
    "wiles_etal_2024": {
        ...
        "key_results": "Treated workers scored 49, 20, and 18 percentage points higher than control on coding, statistics, and prediction tasks. On coding, the treated 95% CI included the data scientist benchmark. On the post-experiment knowledge test (without ChatGPT), the treatment produced no consistent difference. Treated workers also became more overconfident in ChatGPT and worse at predicting which problems GPT-4 can solve.",
    },
    "xu_etal_2025": {
    ...
    "hou_etal_2026": {
        ...
        "key_results": 'On the total post-test (max 27), the guided/prompted arm scored 18.86 vs 15.35 for slide-only control (+3.51 points, d~0.86), while the free/unprompted arm scored 15.73 vs 15.35 for control (d~0.09). The gain was concentrated in open-ended reasoning items (guided 12.41 vs control 9.68 on max 18, d~1.06; omnibus F=7.32, p=0.0011) with no significant effect on multiple-choice recall (F=2.38, p=0.099). The authors conclude that access to AI alone does not improve learning outcomes; the structure of the interaction does.',
    },
    "huang_etal_2025": {
        "setup": 'In October-November 2024, the authors ran a single-site randomized controlled trial with fourth- and fifth-year undergraduate dental students at Wuhan University in China; 192 were recruited and 5 were excluded at eye-tracking calibration, leaving 187 analyzed (ages 20-25, mean 22.53). Both arms received the same instructional videos for a hands-on operative skill. The ChatGPT group (n=94) could additionally consult ChatGPT-3.5 (the free version) during a one-week skill-acquisition period to resolve doubts, verify procedures, and get extra guidance, while the blank control group (n=93) used videos only. After the week, both groups took an operative-skill test on a desktop-VR simulator (auto-scored, 0-100), plus tests of theoretical knowledge, spatial ability (Purdue Spatial Visualization Test), cognitive load (pupillometry), learning motivation, and self-efficacy. No participant compensation or incentives were reported.',
...
ESTIMATE_OVERRIDES = {
    ...
    "henkel_etal_2024__est15": {
        ...
        "notes": "Reported growth-score effect d=0.36 retained (Section 4.1). Schools were randomized (5 treatment, 6 control; Section 3.1), but the paper reports individual-level t-test inference. No school-cluster-valid SE/CI for this d is available. The previously reconstructed independent-student SE and CI are omitted.",
    },
    ...
    "lira_etal_2025__est38": {
        "se": 0.049, "n_total": None, "n_treatment": None, "n_control": None,
        "notes": "Study 3 (February 20, 2026 version), AI practice versus professional-editor feedback. Table S20, supplement p.18, GPT-4o main specification: d=0.20 favoring AI, SE=0.049. The printed table uses the reverse contrast sign. Table S18 reports 2,997 participants across three allocated arms, not an analyzed pair count; Section D1 reports attrition. The analyzed contrast N is not separately verified and is left blank.",
    },
    "lira_etal_2025__est39": {
        ...
        "notes": "Study 4 (2026 version; labeled Study 5 in the 2025 draft). "
                 ...
                 "difference). This pattern is consistent with learning from examples. Effect/SE from "
    ...
    "wiles_etal_2024__est48": {
        ...
        "notes": "No consistent difference on 5 post-experiment knowledge "
                 ...
                 "opposite signs). N varies by question: "
                 "253-573 (Table B7).",
    },
...
ADDITIONAL_ESTIMATES = [
    ...
    _ce("Liu et al. (2026), Direct-answer AI users (Exp 2)",
        ...
        domain="Math", n=466, ci_lo=-0.54, ci_hi=-0.17,
        subgroup="AI use: Direct answers",
        notes="Descriptive, non-randomized split by AI use. Reported 95% CI [-0.54, -0.17] retained (Results, Experiment 2, Fig. 3 discussion, p.7), rather than reconstructing it from the rounded SE."),
    ...
    _ce("Liu et al. (2026), Exp 1 skip rate",
        ...
        domain="Math", n=307, ci_lo=-0.48, ci_hi=-0.02,
        subgroup="Outcome: Skip rate",
        notes="Benefit-oriented sign: higher skipping is worse. The paper reports AI-minus-control d=+0.25 for skip rate, 95% CI [+0.02, +0.48]. The atlas reverses the effect and CI endpoints so positive consistently favors AI."),
    _ce("Liu et al. (2026), Exp 3 skip rate",
        ...
        n=168, ci_lo=-0.72, ci_hi=-0.11, subgroup="Outcome: Skip rate",
        notes="Benefit-oriented sign: higher skipping is worse. The paper reports AI-minus-control d=+0.42 for skip rate, 95% CI [+0.11, +0.72]. The atlas reverses the effect and CI endpoints so positive consistently favors AI."),
    ...
    _ce("Lehmann et al. (2024), prior knowledge above median (post-test)",
        "lehmann_etal_2024", 2.571/4.6, None,
        "ChatGPT access during 45-min Python learning phase",
        "No LLM access",
        "Post-test score (20-question Python coding test)",
        domain="Coding", subgroup="Prior achievement: Above median",
        notes="Above-median level effect = (main + interaction)/4.6 from Table "
              "12, p.23. SE and CI are omitted because the level-effect "
              "variance needs the unreported main-interaction covariance. "
              "The interaction-term SE alone is not the level-effect SE."),
    ...
    _ce("Lehmann et al. (2024), prior knowledge above median (understanding)",
        "lehmann_etal_2024", 0.663/4.6, None,
        "ChatGPT access during 45-min Python learning phase",
        "No LLM access",
        "Post-test, controlling for learning-phase volume (understanding)",
        domain="Coding", subgroup="Prior achievement: Above median",
...
OWN_NUMERIC_FIELDS = (
    "effect_size_sd", "se", "ci_lower", "ci_upper", "n_total",
    "n_treatment", "n_control",
)


def load_own_source() -> dict:
    """Read a reviewed, fixed extraction, never a mutable research output."""
    source = json.loads(OWN_SOURCE.read_text(encoding="utf-8"))
    rows = source["estimates"]
    ids = [r["estimate_id"] for r in rows]
    if source.get("schema_version") != 1 or source.get("review_status") != "reviewed":
        raise ValueError(f"Unreviewed or unsupported own-study source: {OWN_SOURCE}")
    if len(ids) != 9 or len(set(ids)) != 9:
        raise ValueError("The pinned own-study extraction must contain nine distinct estimates")
    if any(not i.startswith("contractor_reyes_2026__") for i in ids):
        raise ValueError("The pinned own-study extraction contains a different study")
    pdf = SITE / "public" / "pdfs" / source["source_document"]
    if hashlib.sha256(pdf.read_bytes()).hexdigest() != source["source_pdf_sha256"]:
        raise ValueError("The linked own-study PDF does not match the reviewed source snapshot")
    return source


def load_own_estimates(source: dict) -> list[dict]:
    """Construct four primary records from the fixed September extraction.

    The five secondary dimensions keep their existing IDs and labels below.
    All nine records receive the same pinned values before output.
    """
    ...
    out = []
    for r in source["estimates"]:
        key = (r["outcome_variable"], r["sample"])
        ...
        out.append(
            dict(
                ...
                study_label=label,
                effect_size_sd=r["effect_size_sd"],
                se=r["se"],
                ci_lower=r["ci_lower"],
                ci_upper=r["ci_upper"],
                learning_domain="General knowledge",
                outcome=outcome_pretty[key],
                outcome_timing=timing_for[key],
...
def apply_numeric_audit(estimates: list[dict], papers: list[dict], own_source: dict):
    """Durable corrections from the September 23 primary-source audit."""
    by_id = {row["estimate_id"]: row for row in estimates}
    own_ids = {row["estimate_id"] for row in estimates
               if row["paper_key"] == "contractor_reyes_2026"}
    if own_ids != {row["estimate_id"] for row in own_source["estimates"]}:
        raise ValueError("Own-study record IDs differ from the reviewed source snapshot")
    for pinned in own_source["estimates"]:
        row = by_id[pinned["estimate_id"]]
        row.update({field: pinned[field] for field in OWN_NUMERIC_FIELDS})
        row["notes"] = (f"{own_source['source_version']} fixed extraction. "
                        f"{pinned['source_locator']} {pinned['derivation_notes']}")

    # Subject concerns the assessed curriculum, not the surrounding program.
    domains = {"learnlm_team_2025": "Math", "chung_etal_2025": "Coding"}
    for row in estimates:
        if row["paper_key"] in domains:
            row["learning_domain"] = domains[row["paper_key"]]

    # Franco Table 2, p.28: raw OLS coefficients on a 0-15 exam scale.
    # Neither a regression SE nor tail shares identify a score SD.
    for suffix, raw_effect, raw_se, arm in (
        ("est85", "+0.078", "0.192", "AI-assisted"),
        ("est86", "-0.234", "0.189", "AI-guided"),
    ):
        by_id[f"franco_etal_2026__{suffix}"].update(
            effect_size_sd=None, se=None, ci_lower=None, ci_upper=None,
            included_in_curated_subset=False,
            quality_flags="No verified exam-score SD; raw coefficient only.",
            notes=f"{arm} versus browsing-only control. Adjusted OLS effect "
                  f"{raw_effect} exam points, SE {raw_se}, on the 0-15 scale "
                  "(Table 2, p.28). SD-unit effect, SE and CI are omitted: "
                  "the previous 1.9-point standardizer was not verified. "
                  "Contrast counts describe the two arms, while the regression "
                  "includes all three arms (572 analyzed, 604 randomized).",
        )

    # Stata output from the pinned arm means/SDs, not an independent-outcome
    # d variance: gain SDs already include the within-person covariance.
    gain_rows = pd.read_csv(BASSNER_PRECISION).to_dict("records")
    if {r["estimate_id"] for r in gain_rows} != {
        "bassner_etal_2026__est59", "bassner_etal_2026__est60"
    } or len(gain_rows) != 2:
        raise ValueError("Unexpected Bassner gain-precision calculation rows")
    for result in gain_rows:
        by_id[result["estimate_id"]].update(
            effect_size_sd=result["exact_effect_size_sd"],
            se=result["se_fixed_denominator"],
            ci_lower=result["ci_lower"], ci_upper=result["ci_upper"],
            notes="Difference in within-person knowledge gains, divided by "
                  f"the pair's pooled pre-test SD ({result['pooled_pre_sd']:.12f}). "
                  "Recomputed in Stata from verified author-data arm statistics "
                  "(Zenodo 20285307; original timestamp/sample convention). "
                  "SE = sqrt(SD_gain_AI^2/n_AI + SD_gain_control^2/n_control) "
                  "/ pooled_pre_SD. Approximate normal 95% CI treats the "
                  "estimated standardizer as fixed and does not adjust for "
                  "tutorial-session clustering. Shared-control contrasts are "
                  "dependent. See code/audit/bassner_gain_precision.do.",
        )
    for suffix, nt in (("est59", 91), ("est60", 88), ("est61", 91),
                       ("est62", 88), ("est63", 88), ("est84", 91)):
        by_id[f"bassner_etal_2026__{suffix}"].update(
            n_treatment=nt, n_control=96, n_total=nt + 96)

    for paper in papers:
        if paper["paper_key"] == "contractor_reyes_2026":
            paper.update(own_source["paper_overrides"])
        elif paper["paper_key"] == "franco_etal_2026":
            paper.update(avg_effect=None, included_in_curated_subset=False)
        elif paper["paper_key"] == "bassner_etal_2026":
            primary = [r["effect_size_sd"] for r in estimates
                       if r["paper_key"] == paper["paper_key"] and not r["is_subgroup"]
                       and r["effect_size_sd"] is not None]
            paper["avg_effect"] = sum(primary) / len(primary)
            paper["summary_strategy"] = paper["summary_strategy"].replace(
                "all SEs use the large-sample d formula.",
                "gain-score SEs use verified within-person gain variances with "
                "the pooled pre-test SD treated as fixed (approximate normal "
                "95% CIs). Other SEs use the large-sample d formula.")


# ── main build ─────────────────────────────────────────────────────────────

def build(*, json_only: bool = False, output_dir: Path | None = None):
    ...
    own_source = load_own_source()
    own_estimates = load_own_estimates(own_source)
    ...
    for est in ADDITIONAL_ESTIMATES:
        ...
        if est_id in DROP_ESTIMATES or pkey in {slugify(p) for p in DROP_PAPERS}:
            continue
    ...
    apply_numeric_audit(estimates_rows, papers_rows, own_source)
    ...
    estimates_df["notes"] = estimates_df["notes"].map(reader_estimate_notes)

    # ── write JSON ────────────────────────────────────────────────────────
    output_dir = output_dir or PAPERS_JSON.parent
    output_dir.mkdir(parents=True, exist_ok=True)
    papers_json = output_dir / "papers.json"
    estimates_json = output_dir / "estimates.json"
    ...
    print(f"WROTE {papers_json} ({len(papers_df)} papers)")
    print(f"WROTE {estimates_json} ({len(estimates_df)} estimates)")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--json-only", action="store_true",
                        help="Do not write the derived research workbook")
    parser.add_argument("--output-dir", type=Path,
                        help="Write preview JSON here instead of src/")
    args = parser.parse_args()
    build(json_only=args.json_only, output_dir=args.output_dir)
