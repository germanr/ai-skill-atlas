  ...
  return (
    <section style={{ paddingTop: 52, paddingBottom: 8 }}>
          ...
          {pooled ? (
            <>
              ...
              <p style={{ fontFamily: F.sans, fontSize: 12, lineHeight: 1.6, color: C.ink3, marginTop: 12 }}>
                Random-effects pooled estimate from {pooled.k} primary estimates in
                student samples, comparing AI with business-as-usual. Learning is assessed
                without AI. Use the Sample filter below to view adult and professional samples.
              </p>
            </>
          ) : (
      ...
      <div className="stats-band" style={{
        ...
        {stat("Study locations", fmt(nCountries))}
      </div>
      <p style={{ fontSize: 11.5, color: C.ink3, lineHeight: 1.6, marginTop: 9 }}>
        Counts cover the full learning dataset, including secondary records. Only {fmt(nStandardized)} of {fmt(estimates.length)} records have a standardized effect; fewer have an uncertainty interval. Study locations count distinct country and multi-country entries. Sample counts may refer to people, sessions, or other observations, so they are not added together.
      </p>
    </section>
  );
}
...
function SourceDetails({ metadata }) {
  ...
  const fields = [
    ...
    ["Source review", provenance.review_status === "reviewed" ? `Reviewed (${provenance.review_basis?.replaceAll("_", " ")})` : "Pending primary-source review"],
      ...
      <main className="wrap" style={{ maxWidth: 1140, margin: "0 auto", padding: "0 28px" }}>
        ...
        <section id="studies" style={{ marginTop: 72 }}>
          <SectionHead
            index="02"
            title="The studies"
            sub="Each card shows one study and its average standardized effect across primary estimates that match the filters. Open a card for the study design, incentives, recorded estimates, and PDF."
            right={(
