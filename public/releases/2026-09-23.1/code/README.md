...
- `C:\Dropbox\Research\ai-learning\data\regression_results_main.dta`: four main Contractor and Reyes estimates.
...
```powershell
Set-Location C:\Dropbox\Admin\website\ai-skill-atlas
python code/build_website_data.py
npm test
npm run build
```

Review the generated diff and build output before publishing. The scripts currently use `C:/Users/greyes/Dropbox` internally, so invoking them through `C:\Dropbox` does not make those internal paths portable. Check their path constants before running on another machine. Check that all source inputs exist: the builder warns and omits the Contractor and Reyes main estimates if it cannot read the Stata file.
...
3. Add the PDF to `public/pdfs/`, using the filename in the paper metadata.
...
The initial annotations distinguish LearnLM's 165 students from its session-level endpoints and the Contractor–Reyes test samples of 211 and 204. These checks use existing local coding, not a new primary-source review. The original JSON counts and effect estimates are unchanged. The aggregate participant headline and mixed-unit sample-size sort have been removed. No claim about unique participants across studies is made.
...
npm test
npm run test:browser
npm run release:create -- --id 2026-09-23.1 --date 2026-09-23 --label "Shareable views, timing filters, sample labels, and source tracking"
